...
Both apps now have real vitest suites (backend: route handlers with `supabaseAdmin` mocked; frontend: `RegistrationForm`/`LoginForm`/`ProfileEditForm` with `fetch` and `next/navigation` mocked) — unit-level, not integration tests against a live Supabase instance or a running app. Neither is wired into CI yet.

`frontend/pnpm-lock.yaml` is kept in sync — `pnpm install` is the only supported way to install frontend deps (never `npm install`, it'll generate a stray `package-lock.json`, gitignored, that masks a real lockfile drift). Note for whoever's running this in a similarly old Node environment: pnpm's CLI throws `ERR_VM_DYNAMIC_IMPORT_CALLBACK_MISSING` under Node 21.7.3 specifically (a Node bug, not a pnpm one) — switch to Node 22+ (`nvm use 24` worked) just to run `pnpm`, everything else about the project still targets Node 20. `frontend/pnpm-workspace.yaml` holds `allowBuilds` (pnpm blocks postinstall scripts by default; `esbuild`/`sharp`/`msw` need theirs) and the `overrides` that used to live in `package.json`'s `pnpm` field (pnpm 11 stopped reading that).
...
`BACKEND_URL` (`frontend/lib/session.ts`, defaults to `http://localhost:3001`) is how any frontend server-side code reaches the backend. Client-side code (`RegistrationForm`, `LoginForm`) instead calls `/api/v1/auth/...` as a bare relative path, which works because `next.config.mjs`'s `rewrites()` proxies `/api/v1/:path*` → `${BACKEND_URL}/api/v1/:path*` — the browser only ever talks to the frontend's own origin, no CORS involved. (This rewrite didn't exist for stories 9431617/9431618; those forms' fetches had nowhere to resolve until it was added 2026-07-26.) Remember rewrites only intercept *incoming* requests to the Next.js server — they don't help outgoing server-side `fetch()` calls, which is why server components/route handlers still need `BACKEND_URL` directly.
...
- `components/developer/PublicProfileFields.tsx` — server-renderable (no `"use client"`) presentational component for the public profile's About/Certifications/Links sidebar block; conditionally renders per `fieldsVisible`. Exports `CertificationsList`/`PublicLinks` subcomponents too. `name` and `experienceYears` deliberately stay inline in `developer/[username]/page.tsx`'s header instead of here — they're tied to that section's avatar/badge layout, not this list-style block.
